      ...
      <div className="h-full flex flex-col justify-between">
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          ...
          <div className="bg-surface-accent p-1.5 rounded-lg border border-border-color">
            <div className="text-[8px] text-text-secondary uppercase tracking-widest mb-0.5">Tiempo Excedido</div>
            <div className="flex items-baseline gap-1">
              <div className={`text-xl font-black ${avgTimeOutside > 60 ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-400'}`}>
                {avgTimeOutside.toFixed(0)}
              </div>
              <div className="text-[9px] text-text-muted">min/avg</div>
            </div>
          </div>

          {/* Riesgo de Merma */}
          <div className="bg-surface-accent p-1.5 rounded-lg border border-border-color">
            <div className="text-[8px] text-text-secondary uppercase tracking-widest mb-0.5">Riesgo Merma</div>
            <div className="flex items-baseline gap-1">
              <div className={`text-xl font-black ${totalLoss > 10000 ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-400'}`}>
                ${(totalLoss / 1000).toFixed(1)}k
              </div>
              <div className="text-[9px] text-text-muted">MXN</div>
            </div>
          </div>

          {/* Desjugue/Degradación */}
          <div className="bg-surface-accent p-1.5 rounded-lg border border-border-color">
            <div className="text-[8px] text-text-secondary uppercase tracking-widest mb-0.5">Alertas</div>
            <div className="flex items-baseline gap-1">
              <div className={`text-xl font-black ${criticalAreasCount > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                {criticalAreasCount}
              </div>
              <div className="text-[9px] text-text-muted">áreas</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2 overflow-y-auto max-h-[220px] pr-2 custom-scrollbar">
          {areas.map((area, i) => {
            ...
            return (
              <div key={i} className={`group border-b ${theme === 'dark' ? 'border-white/5' : 'border-black/5'} pb-2 last:border-0`}>
                <div className="flex justify-between items-end mb-1">
                  ...
                  <div className="flex flex-col items-end">
                    <span className={`text-[10px] font-mono font-bold ${area.severity === 'critical' ? 'text-rose-600 dark:text-rose-400' :
                      area.severity === 'warning' ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'
                      }`}>
                ...
                <div className="h-1 bg-surface-accent rounded-full overflow-hidden flex gap-0.5">
                  <div
                ...
                <div className="flex justify-between mt-0.5">
                  <span className={`text-[7px] ${theme === 'dark' ? 'text-rose-300/60' : 'text-rose-600/60'} opacity-0 group-hover:opacity-100 transition-opacity`}>
                    {area.energyWaste}% exceso energía
                  </span>
                  {area.severity === 'critical' && (
                    <span className="text-[7px] text-rose-600 dark:text-rose-500 animate-pulse font-black uppercase">
        ...
        <div className="mt-2 animate-fade-in">
          <div className={`relative rounded-lg overflow-hidden backdrop-blur-sm border transition-all duration-500 ${coldChainData.alert.severity === 'critical'
            ...
            <div className="relative z-10 p-1.5 flex items-start gap-1.5 transition-all">
              <button
              ...
              <div className="flex-1 min-w-0">
                <h4 className={`text-[9px] font-bold uppercase tracking-wide mb-0.5 ${coldChainData.alert.severity === 'critical' ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-400'
                  }`}>
                  {coldChainData.alert.severity === 'critical' ? 'Riesgo Crítico de Merma' : 'Alerta Térmica Preventiva'}
                </h4>
                <p className="text-[9px] text-text-secondary leading-tight truncate">
